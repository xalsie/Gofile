...
async function multipleUploadExample() {
  ...
  try {
    ...
    console.log('Sequential upload result:', {
      ...
      successfulUploads: sequentialResult.results.filter(r => r.success).length,
    ...
    if (sequentialResult.success) {
      ...
      console.log('- Folder download page:', sequentialResult.downloadPage);
      console.log('- Individual file results:');
      sequentialResult.results.forEach((result, index) => {
        if (result.success) {
          console.log(`   ${files[index].fileName}: ${result.downloadPage}`);
        } else {
